...
public class BotPlayer {
    ...
    public BotPlayer(TetrisModel model)
    {
        this.model = model;
    }
    ...
    public ArrayList<Piece> GetListValidLocations(Piece currentPiece, Boolean[][] grid)
    {
        ...
        for (int o = 0; o < 4; o++) {
            ...
            for (int x = -1; x < model.width+1; x++) {
                ...
                {
                    if (model.IsPieceAtValidLocation(x, y -1, currentPiece.piece, currentPiece.size))
                    {
    ...
    public Piece FindBestMove(HashMap<Piece, Boolean[][]> possibilities)
    {
        ...
        filtered = filterMinHeight(infos);
    ...
    public void RunMoveSet(Piece bestPiece)
    {
        ...
        int diffX = (bestPiece.positionX - model.currentPiece.positionX);
